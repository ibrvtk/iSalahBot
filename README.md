...
3. **[`keyboards.py`](/src/app/keyboards.py)** — клавиатуры _(и reply, и inline)_.
...
2. **salah** — "ежедневник", показывающий какие молитвы уже были сделаны за этот день.
3. **settings** — настройки каждого пользователя. Все настройки _(кроме языка)_ хранятся в формате boolean, где `0` — настройка по-умолчанию, а `1` — противоположная ей. **Исключение:** параметр `"madhab"`, где `0` — это Ханафи, а `1` — Шафии.
4. **stage** — хранит особое состояние пользователя, которое требует подтверждение. Это позволяет иметь одну универсальную данет-клавиатуру _([`kb_yesno`](/src/app/keyboards.py))_ и реализовать всю её логику в одной функции в `match case`.
...
1. ~~Локализация на английский язык.~~ Добавлено. ([#59c03c4](https://github.com/ibrvtk/iSalahBot/commit/59c03c4cce43193883a9017c14b586d786144e6d))
2. Добавление в группы.
3. ~~Показывать в статистике намазы, которые пользователь сегодня отметил как сделанные _(таблица `salahs`)_.~~ Добавлено. ([#6eaec0d](https://github.com/ibrvtk/iSalahBot/commit/6eaec0d307e68d467682ed14b1eded108efbe7ca))
4. Просмотр расписания на завтра.
5. Возможность изменить город.
6. ~~В косметических настройках добавить параметр, который будет называть молитву либо салят _(по-арабски)_, либо намаз _(по-русски)_.~~ Добавлено. ([#59c03c4](https://github.com/ibrvtk/iSalahBot/commit/59c03c4cce43193883a9017c14b586d786144e6d))
