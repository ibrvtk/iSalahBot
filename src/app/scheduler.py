...
from app.data import salah_names, salah_emojis
    ...
    async with connect(DB_DB) as db:
        async with db.execute("""
            SELECT g.user_id, g.timezone_str, g.missed, g.missed_jumuah, t.fajr, t.shuruq, t.ishraq, t.zuhr, t.asr, t.maghrib, t.isha,
                   s.ishraq as hide_ishraq, s.shuruq as rus_shuruq, s.statistics
            FROM general g
        ...
        for row in rows:
            uid, user_timezone_str, old_missed, old_missed_jumuah, fajr, shuruq, ishraq, zuhr, asr, maghrib, isha, settings_ishraq, settings_shuruq, statistics = row
            ...
            for salah_key, salah_time in prayers:
                if user_time_now == salah_time:
                    ...
                    if salah_key == "shuruq" and settings_shuruq == 1:
                        ...
                        salah_name = salah_names['shuruqru']
                    ...
                    text = ""
                    if statistics == 0:
                        reply_markup = await kb_check_salah(salah_key)
                    else:
                        reply_markup = None
                    ...
                    match salah_key:
                        case "fajr":
                            text = (
                                f"{salah_emoji} <b>Ас-саляму алейкум.</b> "
                                f"Время намаза <b>{salah_name}</b> наступило.\n<b>{bot_username}</b>"
                            )
                        case "shuruq":
                            text = (
                                f"{salah_emoji} Время <b>{salah_name}</b> наступило. "
                                f"Совершать намаз до наступления {salah_names['ishraq']}а <b>нельзя</b>.\n<b>{bot_username}</b>"
                            )
                            reply_markup = None
                        case "ishraq":
                            text = (
                                f"{salah_emoji} Время намаза <b>{salah_name}</b> наступило. "
                                f"Его совершение не обязательно, но является <b>больши́м халялем</b>.\n<b>{bot_username}</b>"
                            )
                        case "jumuah":
                            text = (
                                f"{salah_emoji} <b>Сегодня пятница.</b> Нужно пойти в мечеть "
                                f"и сделать <b>{salah_name}-намаз</b>.\n<b>{bot_username}</b>"
                            )
                            if statistics == 0:
                        ...
                        case _:
                            text = f"{salah_emoji} Время намаза <b>{salah_name}</b> наступило.\n<b>{bot_username}</b>"
